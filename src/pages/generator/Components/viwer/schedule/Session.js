// src/pages/generator/Components/viwer/schedule/Session.js
...
const Session = ({ session }) => {
  const hasPositiveScore =
    typeof session.positiveScore === "number" &&
    !Number.isNaN(session.positiveScore);

  return (
    <td
      rowSpan={session.rowSpan}
      className={`${session.color.bg} ${session.color.text} p-auto`}
      key={session.key}
    >
      <p className="text-center text-uppercase fw-semibold m-1 fs-6 lh-sm">
        {session.subject}
      </p>
      <p className="text-center text-uppercase font-monospace m-1 fs-6 lh-sm">
        {session.sequence}
      </p>
